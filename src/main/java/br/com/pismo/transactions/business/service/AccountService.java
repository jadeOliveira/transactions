...
public class AccountService {
    ...
    public AccountDTO findById(Long id) {
        return findOptionalById(id).map(this::entityToDTO).orElseThrow(() -> new AccountNotFoundException());
    }
    ...
    private Account dtoToEntity(final AccountDTO accountDTO) {
        if (StringUtils.isEmpty(accountDTO.getDocumentNumber()) || !StringUtils.isNumeric(accountDTO.getDocumentNumber())) {
            throw new InvalidDocumentNumber();
        }
        return Account.builder().id(accountDTO.getId()).documentNumber(accountDTO.getDocumentNumber()).build();
    }
